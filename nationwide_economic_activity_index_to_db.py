...
from domain.objects import MonthNationwideEconomicActivityIndex
...
MonthNationwideEconomicActivityIndex.get_table().drop(engine, checkfirst=True)
MonthNationwideEconomicActivityIndex.get_table().create(engine)

data = pd.read_csv(config['FILES']['file_dir_path'] + 'economic-activity-index-mexico.csv')

data = map_from_quarter_to_months(data)
data.to_sql(name='month_nationwide_economic_activity_index',con=engine, if_exists='append', index=False)
